...
Question ──► Entity/intent detection
             │
             ▼
        FAISS index (246 SEC filings, ~10K chunks)
             │
             ▼
...
**Key design choices:**
- OpenAI embeddings (`text-embedding-3-small`) — no local model download, works on any machine
...
This is a one-time operation. Point `--corpus-dir` at the folder containing the `.txt` filings:
```bash
...
The index is saved to `data/index/` (gitignored). You only need to rebuild it if the corpus changes.
...
├── data/
│   └── index/               # Generated by indexer.py (gitignored)
│       ├── index.faiss
│       ├── metadata.pkl
│       └── corpus_meta.json
└── docs/
